...
### Motion Control (NEW)
```bash
curl -X POST https://platform.runblob.io/v1/kling/generate -H "Authorization: Bearer YOUR_API_KEY" -H "Content-Type: application/json" -d '{
...
# Motion Control
result = client.generate(
    "Transfer the motion from reference video",
    model="kling_2.6_motion",
    reference_video_url="https://example.com/motion.mp4",
